<template>
  ...
  <div>
  <table id="Greyhound_table" class="center">
  <thead>
    <tr>
      <th>Race Number</th>
      <th>Meeting Name</th>
  ...
  <tbody>
    <tr v-for="item in greyhound_races" v-bind:key="item.race_number">
...
import axios from 'axios';
  const GRAYHOUND_KEY = "9daef0d7-bf3c-4f50-921d-8e818c60fe61"
  export default {
    name: 'Greyhound',
    data() {
      return {
    ...
    methods: {
      ...
      fetch_data() {
        ...
        this.greyhound_races = new Array()
        var count = 0;
        for (var key in this.races) {
          ...
          if (value.category_id == GRAYHOUND_KEY) {
            this.greyhound_races.push(value)
            count += 1;
          }
        }
        this.greyhound_races.sort(function(a, b) {
